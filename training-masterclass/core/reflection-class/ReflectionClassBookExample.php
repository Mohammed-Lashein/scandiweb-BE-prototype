...
class ModuleRunner {
  ...
  public function handleMethod(Module $module, ReflectionMethod $method, array $params) {
    /* suggested naming changes as these var names are a bit not clear
    ...
    if(!$args[0]->hasType()) { 
      ...
      // The above code will do for example : 
  ...
  public function init() {
    ...
    foreach($this->configData as $module_name => $params) {
      ...
      /* 
        A great advice from popps book (And also Eng Mahmoud implemented it in the resolve() in Container class): (for production code, be sure to code defensively: check that the constructor method for each Module object doesn’t require arguments before creating an instance).
       */
